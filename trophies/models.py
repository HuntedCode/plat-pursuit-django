...
class Profile(models.Model):
    user = models.OneToOneField(CustomUser, on_delete=models.SET_NULL, null=True, blank=True, related_name='profile')
    psn_username = models.CharField(max_length=16, unique=True, help_text="PSN Online ID (case-sensitive as per PSN)")
    avatar_url = models.URLField(blank=True, null=True)
    last_synced = models.DateTimeField(default=timezone.now)
    sync_tier = models.CharField(max_length=10, choices=[('basic', 'Basic'), ('preferred', 'Preferred')], default='basic')
...
class Trophy(models.Model):
    ...
    type = models.CharField(max_length=20, choices=[('bronze', 'Bronze'), ('silver', 'Silver'), ('gold', 'Gold'), ('platinum', 'Platinum')])
    earn_rate = models.FloatField(default=0.0)
    icon_url = models.URLField(blank=True, null=True)
    game = models.ForeignKey(Game, on_delete=models.CASCADE, related_name='trophies')
    earned_by = models.ManyToManyField(Profile, through='EarnedTrophy', related_name='earned_trophies')
