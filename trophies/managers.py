...
class BadgeQuerySet(models.QuerySet):
    ...
    def by_type(self, badge_type):
        ...
        Args:
            badge_type: Type of badge ('series', 'collection', 'megamix', 'developer', 'user', 'genre', 'misc')
...
class CommentQuerySet(models.QuerySet):
    ...
    def for_concept(self, concept):
        """
        Get all comments for a concept (concept-level only, no trophy or checklist comments).
        ...
        """
        return self.filter(concept=concept, trophy_id__isnull=True, checklist_id__isnull=True)
    ...
    def for_checklist(self, concept, checklist_id):
        """
        Get comments for a specific checklist within a concept.

        Args:
            concept: Concept instance
            checklist_id: Checklist ID within concept

        Returns:
            QuerySet: Checklist-level comments
        """
        return self.filter(concept=concept, checklist_id=checklist_id)
...
class CommentManager(models.Manager):
    ...
    def for_checklist(self, concept, checklist_id):
        """Proxy to queryset method."""
        return self.get_queryset().for_checklist(concept, checklist_id)
...
class EventQuerySet(models.QuerySet):
    """Custom queryset for the Event model.

    Provides filters for the global Pursuit Feed, the Trophy Feed sub-mode,
    per-profile activity tabs, and visibility against soft-deleted target
    objects. See `docs/architecture/event-system.md` for the full feed
    semantics.
    """

    def for_profile(self, profile):
        """Filter to events authored by a specific profile.

        Excludes system events with `profile=None` (e.g. the Day Zero seed)
        so the per-profile Activity tab does not surface them.
        """
        return self.filter(profile=profile)

    def pursuit_feed(self):
        """Filter to event types that belong in the global Pursuit Feed."""
        from trophies.services.event_service import PURSUIT_FEED_TYPES
        return self.filter(event_type__in=PURSUIT_FEED_TYPES)

    def trophy_feed(self):
        """Filter to the trophy-only sub-feed (platinums, ultra-rare, 100% completions)."""
        from trophies.services.event_service import TROPHY_FEED_TYPES
        return self.filter(event_type__in=TROPHY_FEED_TYPES)

    def since(self, when):
        """Filter to events whose `occurred_at` is on or after `when`."""
        return self.filter(occurred_at__gte=when)

    def feed_visible(self):
        """Filter out events whose target object has been soft-deleted.

        Events are immutable facts. When a source object (e.g. a Review) is
        soft-deleted later, the corresponding Event row is NOT deleted, but
        feed read surfaces should hide it. This method centralizes that
        filtering so every read surface (hub preview, full feed page, profile
        Activity tab, dashboard module) gets it for free.

        For v1, the only event source with a soft-delete pattern is `Review`.
        Future event types whose targets become soft-deletable (e.g. comments
        if comment events are added in v2) should add an exclusion clause
        here.
        """
        ...
        deleted_review_ids = Review.objects.filter(is_deleted=True).values_list('pk', flat=True)

        return self.exclude(
            target_content_type=review_ct,
            target_object_id__in=deleted_review_ids,
        )
...
class EventManager(models.Manager):
    ...
    def pursuit_feed(self):
        """Proxy to queryset method."""
        return self.get_queryset().pursuit_feed()

    def trophy_feed(self):
        """Proxy to queryset method."""
        return self.get_queryset().trophy_feed()

    def since(self, when):
        """Proxy to queryset method."""
        return self.get_queryset().since(when)

    def feed_visible(self):
        """Proxy to queryset method."""
        return self.get_queryset().feed_visible()
