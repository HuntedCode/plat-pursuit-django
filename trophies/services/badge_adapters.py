...
def grant_series_title(profile, series):
    """Grant the series' title (one per series, on ANY group-badge earn in it). Idempotent.
    ...
    UserTitle.objects.get_or_create(
        profile=profile, title_id=series.title_id,
        defaults={'source_type': TITLE_SOURCE, 'source_id': series.id},
    )


def revoke_series_title_if_orphaned(profile, series):
    ...
    still_earned = UserGroupBadge.objects.filter(
        profile=profile, group_badge__series=series, status='earned',
    ).exists()
    if not still_earned:
        UserTitle.objects.filter(
            profile=profile, title_id=series.title_id, source_type=TITLE_SOURCE,
        ).delete()


def emit_badge_earned(profile, group_badge):
    ...
    logger.info("badge_earned: profile=%s group_badge=%s", profile.id, group_badge.id)
