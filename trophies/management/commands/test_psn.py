...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            nargs="?",
            default="abu_abu",
            help="PSN Online ID to test (default: abu_abu)",
        )
    ...
    def handle(self, *args, **options):
        ...
        if not token or len(token) != 64:
            self.stdout.write(
                self.style.ERROR(
                    "Set NPSSO_TOKEN in .env (must be 64-char NPSSO cookie from playstation.com)"
                )
            )
            return
        ...
        try:
            ...
            #game_title = psnawp.game_title('PPSA23060_00', PlatformType('PS5'), account_id=user.account_id)
            #print(game_title)
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "v3.0.0 API test successful! Data ready for model syncing."
                )
            )
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"Auth/Token Error: {str(e)} - Regenerate NPSSO token via browser DevTools."
                )
            )
            ...
            if e.response.status_code == 429:
                self.stdout.write(
                    self.style.WARNING(
                        "Rate Limit Hit (429) - Wait 15 min; library auto-retries on next call."
                    )
                )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"HTTP Error: {str(e)} - Check token or PSN username."
                    )
                )
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"Unexpected Error: {str(e)} - Verify library version and token."
                )
            )
            self.stdout.write(
                self.style.WARNING('Run "pip show psnawp" to confirm v3.0.0.')
            )
