...
class BadgeDetailTourDismissAPIView(APIView):
    """
    POST /api/v1/tutorial/badge-detail/dismiss/

    Marks the Badge Detail coach-marks tour as completed or skipped for
    the authenticated user's profile.
    """
    permission_classes = [IsAuthenticated]
    authentication_classes = [SessionAuthentication]
    ...
    def post(self, request):
        ...
        profile.badge_detail_tour_completed_at = timezone.now()
        profile.save(update_fields=['badge_detail_tour_completed_at'])

        track_site_event(f'badge_detail_tour_{action}', f'step_{last_step}', request)
