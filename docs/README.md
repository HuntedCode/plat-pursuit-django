...
| [Rebuild Playbook & Record](design/rebuild/rebuild-playbook.md) | **Start here for any page.** The site-wide rebuild is COMPLETE (2026-09); this is the per-page record (read the table, never a count above it) and it carries the shared decisions every rebuilt page inherits: stacked structure, header card, segmented tabs, the depth-pass surface ladder, toolbars, premium motion, HTMX swaps, modals. Indexes the authoritative docs. |
| [Career: Reference Standard](design/rebuild/career-reference-standard.md) | **The finished-quality bar for the site.** Every page is measured against Career's polish + coherence: design tokens, mobile-first fit, premium motion, performance, interaction, URL/state — plus the "what would Google/Apple do here?" polishing lens. |
| [Leaderboards: Section Rebuild](design/rebuild/leaderboards-rebuild.md) | **SECTION CLOSED (2026-08-19), and PARTLY REVERSED.** What shipped: boards live ON their entity, and all four surfaces converged onto one shared board. What did NOT: the hub was to become a discovery layer of thin directories (Global / Game / Badge / Job) -- those three directories were built and then REMOVED, so `/leaderboards/` is one destination with no rail. Country is a FILTER not a board; badge XP is renamed Badge Points to separate it from Career XP; `/jobs/` + job detail land in Browse. Includes the backend audit's findings and the Lane B cutover sequencing. |
| [Games and Trophy Lists IA](design/games-and-trophy-lists-ia.md) | **Vocabulary + IA decision (2026-08).** Game = the IGDB-anchored Concept; Trophy List = one platform list (the `Game` model, NOT renamed). Four canonical pages, the Game-detail wrapper pattern (URL-addressable list switcher among concept tabs), the shared trophy-grid partial rendered on both detail pages, the `display_list_name` helper chain over PSNTitleObservation, and the neutral-URL naming insurance for Xbox/Steam. Every rebuilt game surface inherits this. |
...
| [Binder Surface](design/binder-surface.md) | **SUPERSEDED** by the [Badge Medallion Case](reference/badge-medallion.md) on `/collection/` (2026-07). The `/design/binder/` lab itself was removed in the 2026-08 design-lab strip; the doc records the six views, 3D page-flip, and technical learnings. |
...
| [Stats Page](design/stats-page.md) | The `/stats/` stats screen (100+ stats). **HIDDEN since 2026-08 and out of scope for the site-wide rebuild** -- `/stats/` 302s to Home, the legacy `/my-stats/` 301s into that; returns later as its own tool. |
...
| [Stats Page Inventory](reference/stats-page-inventory.md) | Quick reference of every stat the hidden stats page displayed (12 sections, 120+ stats). Inventory of a parked surface, not a live one. |
