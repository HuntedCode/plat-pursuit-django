...
**Twenty pages are finished to the standard: Career, Collection, Badges, Badge Detail, Milestones, Titles, Plat Cards, Browse Games, Game Detail, Recently Added, Monthly Recap (archive + ceremony), Game Lists (browse + detail), Genres & Themes (list + detail), Franchises (list + detail), Companies (list + detail).**
...
| **Game Lists** | `/community/lists/`, `/community/lists/<id>/` | ✅ | **Complete.** Browse rebuilt on the Franchises shape (`HtmxListMixin` partial swap, infinite scroll, sticky mini-bar, staggered reveal) and the tile is the SHARED `.pp-gtile` with two additions rather than a parallel component: a **cover mosaic** that composes itself around however many covers a list has (1 fills, 2 splits, 3 gives the first a column beside a stacked pair, 4 quarters), and an author line. A list reads as a collection instead of a game at a glance. Both pages were scaling their query count with their own content: browse rendered `GameList.first_game_image`, a PROPERTY that runs its own query per card (**23 queries for 20 lists -> 3, flat**), and detail rendered the IGDB-first `display_image_url` per item off `select_related('game')` alone, without deferring `raw_response` (**82 queries for a 40-game list -> 2, flat**). The browse tile also read `game.title_image` directly, making lists the one surface disagreeing with the site about what a game's cover is. Detail's nine sort LINKS (nine full reloads of an unpaginated list) became one toolbar select swapping just the items. Tested (`test_lists_browse`, 17 cases incl. both query-flatness pins and the privacy rule). **Deferred:** My Lists (`/my-lists/`) — a management surface, not a browse page, and its own pass. |
