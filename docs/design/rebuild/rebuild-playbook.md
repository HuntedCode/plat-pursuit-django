...
**Eleven pages are finished to the standard: Career, Collection, Badges, Badge Detail, Browse Games, Game Detail, Recently Added, Genres & Themes (list + detail), Franchises (list + detail).**
...
| **Challenges / Company / Game Lists / other Browse pages** | various | 🟡 | Header card adopted, but **no** depth pass / segmented switcher / premium motion. Header-aligned only. The shared `game_cards.html` card + flag/badge filter controls they include were upgraded to `.pp-gcard`/house style by the Browse Games pass (not separately verified). **Company detail's group cards already inherited the `.fgroup` rebuild** (shared `game_groups_list.html`); its **shell** (role tabs + merger/community-rating header) still needs its pass. The **Company list rebuild can adopt the `.pp-gtile` grouping tile + the materialized-cover pattern** (`recompute_tag_covers` — needs a Company branch added), following Franchises ~1:1. |
