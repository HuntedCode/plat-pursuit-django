...
**Fourteen pages are finished to the standard: Career, Collection, Badges, Badge Detail, Milestones, Browse Games, Game Detail, Recently Added, Genres & Themes (list + detail), Franchises (list + detail), Companies (list + detail).**
...
| **Titles** | `/titles/` | ⛔ | Not rebuilt. **Half-gutted by the Lane 2 teardown**: the Milestone Titles group is gone (the legacy engine that fed it was deleted), so the page now renders only Badge Titles + Special (one-off awards surviving as `UserTitle(source_type='milestone')` with no source row). Next up for a rebuild. |
