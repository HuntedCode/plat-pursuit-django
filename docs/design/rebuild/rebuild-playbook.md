...
**Twelve pages are finished to the standard: Career, Collection, Badges, Badge Detail, Browse Games, Game Detail, Recently Added, Genres & Themes (list + detail), Franchises (list + detail), Company list.**
...
| **Company detail** | `/companies/<slug>/` | 🟡 | Header card adopted; its **group cards already inherited the `.fgroup` rebuild** (shared `game_groups_list.html`). Remaining: the **shell** (role tabs + merger chain + community-rating header) needs its from-scratch pass, and the related-companies surface can adopt the now-materialized `Company.representative_game` FK / `.pp-gtile`. Follows Franchise detail ~1:1. |
