...
**Eighteen pages are finished to the standard: Career, Collection, Badges, Badge Detail, Milestones, Titles, Plat Cards, Browse Games, Game Detail, Recently Added, Monthly Recap (archive + ceremony), Genres & Themes (list + detail), Franchises (list + detail), Companies (list + detail).** (Game Lists was rebuilt and then hidden pending a revamp, so it is not counted.)
