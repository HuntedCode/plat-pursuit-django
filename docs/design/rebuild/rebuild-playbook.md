...
| **Titles** | `/titles/` | ✅ | **Complete.** From-scratch rebuild on the NEW grouping-badge system (it read the legacy `Badge` model, so `badge_series` titles rendered NOWHERE). A title belongs to a SERIES -- clear it and the title is yours -- so the page is the record of what you've proven, not a second Collection. **Nameplate** hero (the title you're wearing, display face, `aria-live`) inside the accented `.pp-head-cascade` header over a count-up `.scard` row -> site-standard right-aligned `.pp-switch` (**Yours** / **Within reach** / **All**, counts as chip captions) over three SSR panels toggled with `wireTablist` + `igniteTab` + `slideViewIn` + `syncViewParam`. **ONE row component** serves all three views -- only the trailing slot changes -- with the medallion deliberately small (`--sz: 42px`) since Collection is where medallions are the object. **Within reach** is the motivating slice: unearned titles ranked CLOSEST-FIRST off the materialized `SeriesBadgeStanding.progress_bp` (a read, not a computation) with a Horizon + "N of M stages"; **All** carries holder counts as social proof. Equip is in-place (`/api/v1/equip-title/` unchanged) with a pending state, and both the "Wearing" marker and the button ship on every row so equipping is a single `.is-worn` toggle that also flips the previously-worn row. Surviving one-off awards from the retired milestone engine show as Special. Five bounded queries (whale-safe); depth + sr-only headings + 375 pass applied; tested (`tests/engine/test_my_titles.py`). |
