...
| 4 | **Backfill genre/theme read-models** — materializes `Genre/Theme.representative_game` (the tile + detail-hero cover) AND `.related_tags` (the detail-page related rail) for the rebuilt `/genres/` list + `/genres|themes/<slug>/` detail pages. Until it runs, tiles render the neutral placeholder and detail pages show no related rail. Then keep fresh via the daily cron below. | `python manage.py recompute_tag_covers` | Launch (after migrations) | Yes (recomputes from scratch; stable pick) | `/genres/` tiles show the tag-glyph placeholder; tag detail pages omit the related-tags rail | ☐ |

> **Ordering:** #1 and #2 depend on the STEP 2 schema migrations (`UserBadge.status`/`earn_rank`, `Badge` rarity fields); #3 depends on the `SeriesBadgeStanding.group_progress` migration (`0276`); #4 depends on the `Genre/Theme.representative_game` + `.related_tags` migrations (`0277`, `0278`). All run after migrations; run order between them doesn't matter. **Note:** #3 was verified on beta (evaluate_badges backfills group_progress as expected) — it still needs a run on prod at cutover.
