...
| 12 | **Backfill the PP Score board** — `ProfilePPStanding` (migration `0335`) is created EMPTY, so the PP Score tab renders an empty wall until this runs. LESS urgent than #10 because PP Score is not the default tab: the landing still opens on Shovelware Free, so an empty PP board is one quiet tab rather than the section's front door. Needs NO prior step — it reads `Trophy.trophy_earn_rate`, PSN's own figure written during sync, so nothing in `nightly` has to precede it. Long job (it aggregates every linked hunter's earned trophies): use a one-off Render job, not an interactive shell. `--dry-run` reports the row count without writing; a budget-capped run resumes from a cursor rather than restarting, so it is safe to bound. After this the nightly keeps it fresh. | `python manage.py recompute_pp_standings` (preview with `--dry-run`) | Launch (after migrations) | Yes (recompute from scratch; skips unchanged rows) | The PP Score tab is an empty wall | ☐ |
