...
| K | **Rarity ratchet: migrate, schedule the cron, then seed the floors** — migration `0286_rarity_floor` adds `rarity_floor_pct` to `GroupBadge` and `Title` (both nullable, no rewrite). They land **NULL**, which reads as "no floor" and grades live — correct, not broken, but it means nothing is protected from demotion until the first run. Seed it immediately after migrate rather than waiting for the 05:15 UTC cron, because every day before the first run is a day a grade can silently drop. Idempotent, and it only ever moves a floor DOWN. **Also add the cron** (see [cron-jobs](../../guides/cron-jobs.md)). | `python manage.py recalc_rarity_floors`, then create the Render Cron Job daily 05:15 UTC | With the rarity primitive deploy | ☐ |
