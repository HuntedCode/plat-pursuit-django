...
| 10 | **Backfill the Shovelware Free board** — `ProfileTrophyStanding` (migration `0332`) is created EMPTY, and the board that reads it is now the DEFAULT tab at `/leaderboards/`. Until this runs, a bare `/leaderboards/` renders an empty wall to every visitor, signed in or not — this is not a degraded corner of a page, it is the landing state of the section. **Run it in the same maintenance window as the migration, before the deploy is announced.** `update_shovelware` must have run at least once first (it is now `nightly` step 1), or every game reads as clean and the board is a copy of Trophies. Long job: use a one-off Render job, not an interactive shell. `--dry-run` reports the row count without writing. After this, the nightly keeps it fresh and no further runs are needed. | `python manage.py update_shovelware`, then `python manage.py recompute_clean_standings` (preview with `--dry-run`) | Launch (after migrations, BEFORE announcing) | Yes (recompute from scratch; skips unchanged rows) | `/leaderboards/` — the DEFAULT board — is an empty wall for every visitor | ☐ |
| 11 | **Migrations `0333_total_trophies_raw` + `0334_profile_board_indexes_on_raw`** — 0333 adds `Profile.total_trophies_raw` (unfiltered trophy total) and fills it; 0334 moves the Trophies board's two partial indexes onto it. They are SPLIT deliberately: 0334 needs `atomic = False` for CONCURRENTLY, and under that Django replays from operation 0 after a partial failure — bundled, the replay would hit `AddField` and die on "column already exists" with the operator mid-deploy. **Needs no backfill task**: every earned trophy is one of the four type counters, which are already unfiltered, so the fill is one arithmetic UPDATE inside 0333, narrowed to profiles that hold a trophy (the other ~250k rows are already correct at 0 and writing them would row-lock the table against live sync). Old code is what protects the gap between the two migrations — the board keeps reading `total_trophies` until the deploy swaps it. Listed here so it is not mistaken for something needing a follow-up run. **It DOES rebuild two indexes on `Profile` CONCURRENTLY** (~300k rows), so expect it to take minutes; `atomic = False`, and a build that fails partway leaves an INVALID index that must be dropped by hand before re-running — the migration docstring has the exact SQL. | (auto, on `migrate`) | Launch (with the leaderboard work) | Yes | Nothing; but if the index build fails and is not cleaned up, `trophy_rank` seq-scans a 48-column table on every authenticated page view | ☐ |
| 12 | **Backfill the Rarity Score board** — `ProfileRarityStanding` (migration `0335`) is created EMPTY, so the Rarity Score tab renders an empty wall until this runs. LESS urgent than #10 because Rarity Score is not the default tab: the landing still opens on Shovelware Free, so an empty Rarity Score board is one quiet tab rather than the section's front door. Needs NO prior step — it reads `Trophy.trophy_earn_rate`, PSN's own figure written during sync, so nothing in `nightly` has to precede it. Long job (it aggregates every linked hunter's earned trophies): use a one-off Render job, not an interactive shell. `--dry-run` reports the row count without writing; a budget-capped run resumes from a cursor rather than restarting, so it is safe to bound. After this the nightly keeps it fresh. | `python manage.py recompute_rarity_standings` (preview with `--dry-run`) | Launch (after migrations) | Yes (recompute from scratch; skips unchanged rows) | The Rarity Score tab is an empty wall | ☐ |
| 13 | **Repair `ProfileGame.progress` rows outside 0-100, then re-award what they blocked** — PSN can hand us a self-contradictory payload during a trophy-list restructure (The Long Dark NPWR13317_00: earned=88 against defined=76, grade-weighted to `progress=111`). The sync now refuses such a value, but rows poisoned before the fix only self-heal on a sync that WALKS, and a hunter who finished the game and earns nothing on it again may never trigger one. This is not cosmetic: `progress == 100` is an equality test for badge `full_complete`, contract completion, the plat-card gate, `Profile.total_completes` and `Game.full_completion_count`, so each poisoned row is a hunter silently denied credit for a game they completed. 3 rows at the time of writing (2 on The Long Dark at 111, 1 on Neverwinter at 101). Clamping to 100 matches the fix's own rule for a payload whose earned count covers the defined set; spot-check each row against its EarnedTrophy rows first, since a row with unearned trophies should take the floored count-based value instead. Then re-evaluate those profiles so the credit actually lands. | `ProfileGame.objects.filter(progress__gt=100)` in a shell to review, then `.update(progress=100)`, then `evaluate_badges` + `process_contracts` scoped to the affected profiles | Now (with the `fix/psn-progress-correction-gate` deploy) | Yes (the filter is empty on a re-run) | Affected hunters keep a completion that counts for no badge, contract or plat card, and shows an impossible percentage on the game page | ☐ |

> **#10 depends on migrations 0332-0334 AND on at least one `update_shovelware` run** (now `nightly`
> step 1). It is also the one with a user-visible failure mode if it is skipped or run out of order — it backs the
> DEFAULT leaderboard tab, so getting it wrong is a blank landing page rather than a stale figure.
>
...
>
> 1. **ADD `nightly` (04:00 UTC) FIRST.** It runs SEVEN steps in dependency order: `update_shovelware`
>    -> `recompute_clean_standings` -> `evaluate_badges --all` -> `detect_dlc_and_refresh` ->
>    `process_contracts --all --incremental` -> `recompute_milestones` -> `audit_badge_coverage`.
> 2. **THEN delete the five standalone entries** for the commands it now owns -- including the 04:00
>    `update_shovelware` entry, which is the one most easily missed because `nightly` shares its slot.
>    Leaving it runs the detector twice, concurrently, and reinstates exactly the undefined-order race
>    with `recompute_clean_standings` that folding it in removed. Add-before-delete, so there is never a
>    window with neither. Deleting them first would stop every job at once, silently -- a cron that does
...
- [ ] **Replace the FIVE nightly Render entries with ONE `nightly` entry at 04:00 UTC.** Delete
      `evaluate_badges --all` (04:00), `detect_dlc_and_refresh` (04:30), `audit_badge_coverage` (05:00),
      `recompute_milestones` (05:30) and -- since 2026-09 -- `update_shovelware` (04:00); `nightly` runs
      all of them in dependency order. `update_shovelware` is the one most easily missed, because it
      SHARES the 04:00 slot with `nightly`: leaving it also reinstates the undefined-order race with
      `recompute_clean_standings` (step 2), which reads the flags it writes. Leaving the old entries in place is not
...
`populate_title_ids` (02:00), `recalc_earn_rates` (03:00), `recalc_profile_counters` (03:30) and
`recompute_tag_covers` (03:45) remain separate entries whose ordering is implied by their times.

**UPDATED 2026-09:** `update_shovelware` (04:00) has LEFT this list -- it is `nightly` step 1, and
`recompute_clean_standings` (step 2) reads the flags it writes. It was folded in precisely because
wall-clock ordering could not express that: both sat at 04:00, so the order between them was undefined.
It leads the chain so its start time is unchanged, which `evaluate_contract_candidates` (04:45) still
depends on. **Folding them in is a tracked follow-up** (see the FOLLOW-UP block in
[cron-jobs.md](../../guides/cron-jobs.md)); it was left out here to keep the change to one subsystem.

**UPDATED 2026-08:** `recompute_milestones` is no longer in this list -- it is step 7 of `nightly`. Its
