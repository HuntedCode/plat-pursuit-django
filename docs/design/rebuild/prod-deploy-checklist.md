...
| 11 | **Migration `0333_total_trophies_raw`** — adds `Profile.total_trophies_raw` (unfiltered trophy total) and moves the Trophies board's tiebreak, membership rule and BOTH partial indexes onto it. **Needs no backfill task**: every earned trophy is one of the four type counters, which are already unfiltered, so the fill is one arithmetic UPDATE inside the migration and the column is populated before the indexes are built. Listed here so it is not mistaken for something needing a follow-up run. **It DOES rebuild two indexes on `Profile` CONCURRENTLY** (~300k rows), so expect it to take minutes; `atomic = False`, and a build that fails partway leaves an INVALID index that must be dropped by hand before re-running — the migration docstring has the exact SQL. | (auto, on `migrate`) | Launch (with the leaderboard work) | Yes | Nothing; but if the index build fails and is not cleaned up, `trophy_rank` seq-scans a 48-column table on every authenticated page view | ☐ |
...
> **#10 is the one with a user-visible failure mode if it is skipped or run out of order** — it backs the
