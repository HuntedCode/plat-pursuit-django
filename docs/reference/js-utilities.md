...
| Method | Parameters | Purpose |
|--------|-----------|---------|
| `DetailModal(el, opts)` | HTMLElement, `{closeSelector?, autoOpenDelay?, armed?, onDismiss?, seenKey?, onSettled?}` | Open / close / Escape / focus trap / `is-closing` exit for a `.pp-detail-modal`, plus the one-shot "mark it seen" write |
...
lesson below was learned on one of them and carried to the other by hand. Returns `{open, close, isOpen}`.
...
**`armed` defaults to `el.hasAttribute('data-auto')`** and `onDismiss` fires **at most once**, on the
