...
| `contract_detection:last_run` | None | Where the next `process_contracts --all --incremental` pass STARTS: it narrows to `Contract.updated_at > this - CURSOR_GRACE` (1 minute). The stored value is the run's **start** instant, not its end. Advanced on every incremental-mode run -- including a quiet one that finds nothing *changed*, and including a forced FULL pass, which advances **both** keys. **Not** advanced by `--dry-run`, `--contract`, `--user`, or a run that finds no live Contracts at all. |
| `contract_detection:last_full_run` | None | When the last FULL `process_contracts` pass ran (again, its start instant). It is what the 7-day `FULL_SWEEP_INTERVAL` is measured against, and the only key whose *age* can force a full pass. Advanced only by a run that actually was full. (A full pass is also forced whenever *either* key is missing or unreadable, so this is not the sole input to that decision.) |
