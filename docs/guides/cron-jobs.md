...
- **What it does**: Materializes two per-tag read-models on every `Genre` / `Theme`. (1) **`representative_game`** — the tag's browse-tile + detail-hero cover: a CONTRACT game (curated Job-Board entry, so tiles favour recognizable titles) with a STABLE per-tag variety shuffle (a hash of tag+game id, so adjacent tiles differ but a tile never reshuffles between page loads), bounded to the most-recent `POOL_CAP` (50) contract members; falls back to the most-recent member with real cover art, then any most-recent member. (2) **`related_tags`** — the detail-page rail: the top-`RELATED_N` (6) OTHER same-type tags ranked by game co-occurrence (genres/themes whose games overlap this one's the most), stored as an ordered slug list. `bulk_update`s only the tags whose picks changed.
- **Why it exists**: Both were originally per-request queries whose cost grows with the catalogue / contract-catalogue size (a correlated cover subquery per tile; a co-occurrence GROUP BY over a tag's games per detail load). Materializing them off the request path makes the pages read them O(1), so they scale regardless of how large contracts or genres get. Slow-changing derived data → a textbook denorm/read-model (same pattern as `recalc_earn_rates`' community stats).
