...
| `nightly` | **The nightly maintenance run, one cron entry.** Executes the badge chain in DEPENDENCY order (evaluate -> DLC sweep -> entrants recompute -> coverage audit). Each step is isolated so one failure does not cancel the rest, and the command exits non-zero if any failed. Add nightly work as a STEP here, not as a new cron entry. | `--dry-run`, `--only <label>`, `--skip <label>` | `python manage.py nightly --dry-run` |
| `recalc_board_entrants` | Recompute the denormalized `BadgeSeries.entrants` / `Job.entrants` counts the board directories gate and sort on. RECOMPUTE-FROM-SCRATCH, never incremented. Must run AFTER `evaluate_badges`, which writes the standings it counts -- `nightly` sequences that. | `--dry-run` | `python manage.py recalc_board_entrants --dry-run` |
