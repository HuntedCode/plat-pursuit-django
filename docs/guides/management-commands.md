...
| `review_shovelware_blacklist` | Read-only review sheet of currently-blacklisted developers (shovelware proportion, dominant genres/themes, sample games, count flagged) to decide whitelist candidates. Sorted by impact. | `--samples N`, `--limit N`, `--include-whitelisted` | `python manage.py review_shovelware_blacklist` |
