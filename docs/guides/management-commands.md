...
PlatPursuit has **98 custom management commands** spread across 7 Django apps: `trophies` (62), `core` (23), `notifications` (5), `users` (4), `milestones` (2), `art_reveal` (1), `fundraiser` (1). All commands follow the standard Django pattern and are invoked with `python manage.py <command_name>`. Many support `--dry-run` for safe previewing before applying changes.
...
| `process_contracts` | Backfill Contract reach-detection from EXISTING completion data, so a completion earned before a Contract existed becomes claimable without the hunter re-syncing that game. Detection only: stamps `EarnedContract.*_reached_at`, grants NO XP, never auto-accepts. `--all --incremental` is nightly step 3. **`--contract <slug>`** sweeps ONE live Contract across every candidate profile -- the targeted additive counterpart to `reconcile_contracts --contract`, for the moment a Contract is published or credit has just been revoked in a re-key; without it that Contract waits for the nightly. Refuses a DRAFT contract (the reached stamp makes it claimable and an un-publish does not take that back -- deliberately the opposite of `reconcile_contracts`, which ignores `is_live`), and never writes the incremental watermark. Whale-safe: candidate-filters per Contract in the DB rather than scanning the userbase. | `--user <psn_username>`, `--all`, `--contract <slug>`, `--dry-run`, `--incremental` | `python manage.py process_contracts --contract myst-original` |
| `recompute_job_xp` | Rebuild the `ProfileJobXP` cache (and the `ProfileCareerStanding` roll-up that rides it) by re-summing the immutable `ContractXPGrant` ledger in the DB. Repairs drift after data edits or a manual ledger fix, and is the BACKFILL to run after any change to how Pursuer Level is defined -- the standing stores a materialized copy, so rows written before such a change keep the old rule and nothing errors. `--all` sweeps profiles with grants OR a standing row: a grants-only sweep skips hunters whose grants were revoked, who keep a zeroed standing with no ledger behind it. | `--user <psn_username>`, `--all` | `python manage.py recompute_job_xp --all` |
| `reconcile_contracts` | **The only subtractive path in the Contract engine.** Revokes credit that current membership no longer supports -- after a Concept split, a re-anchor, or a lost trusted match, the games that left a Contract leave stamped credit (and banked XP) behind, because detection and acceptance are both forward-only. Deletes the `EarnedContract` (grants cascade) and rebuilds `ProfileJobXP` + `ProfileCareerStanding` per hunter. Revokes only when NEITHER tier detects, so a DLC knocking a hunter off 100% never costs them XP. **Preview is the default**; writing takes `--apply`. REFUSES to apply against a contract that resolves to zero members (a mid-rematch or mistyped `igdb_id` looks identical to a split, and would revoke every earner) unless `--force-empty`. Every revoke is logged before the delete. One named Contract at a time, staff-run, never on cron. | `--contract <slug>` (required), `--user <psn>`, `--apply`, `--force-empty` | `python manage.py reconcile_contracts --contract myst-2020` |
...
| `announce_contracts` | Post newly published Contracts to Discord, grouped by JOB, with a link to the board with Latest applied. **Silent when nothing is new** (most runs). "New" = `is_live=True` + `went_live_at` stamped + `announced_at` null, which excludes staged/review-queued candidates structurally. It does NOT exclude the ~1,000 launch contracts: they carry real `went_live_at` stamps (checked on prod 2026-09-10), so the first run refuses them over `MAX_WAVE` and `--baseline` is a required cutover step. Idempotent via the `Contract.announced_at` COLUMN, set by a confirmed 2xx AND by `--baseline`; the separate `announcement_posted` flag is the one that means "actually posted", and the Career new-contracts modal reads it. Refuses a wave over 40 without `--force` (a bulk publish would post a wall); use `--baseline` once after the launch seed to record the backlog as already known. `--test-webhook` deliberately does NOT stamp, so a preview cannot consume a wave. See [Cron Jobs](cron-jobs.md#announce_contracts). | `--dry-run`, `--test-webhook`, `--limit N`, `--baseline`, `--force` | `python manage.py announce_contracts --dry-run` |
...
| `recompute_clean_standings` | Rebuilds `ProfileTrophyStanding` from EarnedTrophy, excluding games flagged as shovelware -- the store behind the Shovelware Free board. Recompute-from-scratch and the table's ONLY writer: shovelware is a property of the game, so one re-flagged game invalidates every hunter who played it, which no per-row signal can see. Skips unchanged rows; a budget-capped run resumes from a cursor rather than restarting. | `--dry-run`, `--chunk-size`, `--max-minutes`, `--profile-ids` | Runs as `nightly` step 2; `--profile-ids <id>` for ad-hoc repair |
| `recompute_rarity_standings` | Rebuilds `ProfileRarityStanding` -- the store behind the **Rarity Score** board -- by summing `100 / trophy_earn_rate` over each linked hunter's 1,000 rarest BASE-GAME trophies. Recompute-from-scratch and the table's ONLY writer: a hunter's score depends on WHICH trophies are their rarest thousand, so one new trophy can displace another out of the set and move the total by more than its own worth. Aggregates by rate rather than sorting, so cost is bounded by the ~1,000 distinct rate values, not by library size. | `--dry-run`, `--chunk-size`, `--max-minutes`, `--profile-ids` | Runs as `nightly` step 3; `--profile-ids <id>` for ad-hoc repair |
...
| ~~`update_shovelware`~~ | **`nightly` step 1** | Surgical shovelware reconciliation (idempotent drift correction). **No standalone cron entry since 2026-09** -- it shares the 04:00 slot with `nightly`, so a separate entry runs it twice concurrently and reinstates an undefined-order race with `recompute_clean_standings`. See [cron-jobs.md](cron-jobs.md). |
| ~~`recompute_clean_standings`~~ | **`nightly` step 2** | Rebuilds `ProfileTrophyStanding`, the store behind the Shovelware Free board (the DEFAULT tab at `/leaderboards/`). Reads step 1's flags and is skipped if it failed. **No standalone cron entry.** |
| ~~`recompute_rarity_standings`~~ | **`nightly` step 3** | Rebuilds `ProfileRarityStanding`, the store behind the Rarity Score board. Reads PSN's `trophy_earn_rate` (written at sync), so unlike step 2 it depends on NOTHING earlier in the chain. **No standalone cron entry.** |
...
| `reconcile_contracts` | Revoke Contract credit orphaned by a Concept split / re-anchor (preview by default, `--apply` to write) |
...
without one is simply absent from the page); contracts whose games are complete but which are not
stamped reachable (the gap `process_contracts`, nightly step 3, exists to close); and the inverse --
contracts a hunter is credited for but no longer qualifies for, which is what a Concept split or
re-anchor leaves behind (fix with `reconcile_contracts --contract <slug> --apply`).
