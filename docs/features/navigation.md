...
| **Avatar dropdown** | Profile · Membership (if member) · Settings · Mod Center (mods/admins) · Logout |
...
Streamlined to essentials: Profile, Membership (members only), Settings, Logout. The theme toggle went with light mode (2026-08), and the 2026-08 staff strip-down removed every staff link: staff pages are bookmark-reached and cross-link each other, so the menu reads the same for every signed-in hunter.
