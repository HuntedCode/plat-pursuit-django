...
(A legacy Welcome Tour once pointed new users at the sub-nav strip on first visit; that tour system was removed in the chrome rebuild and will be rebuilt from scratch if reintroduced.)
...
**Performance**: Only the active tab's lazy modules load on page init. Other tabs load on first activation. Cache keys include a settings hash so different configurations are cached independently. Mutation points (badge sync, list changes) invalidate the dashboard cache.
