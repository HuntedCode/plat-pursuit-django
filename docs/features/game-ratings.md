...
| **The wall** (`.pp-rwall` → `.pp-rcard`) | The card is keyed on the **recommendation**: its left edge takes that colour (cyan when a rating predates the field), the glyph is stamped over the art at 46/58px, and the label sits in the body as plain text. Three signals, one `--rec-c`, so they cannot disagree. The edge used to encode the star tone — a different fact, since the stars rate the game and the recommendation rates the platinum, so a card could be edged red for a 2-star game the hunter still says to do. One **wide** card per rating (1 column on mobile, `auto-fill minmax(420px)` from `md:`): a full-height **landscape art panel** bled to the card's left edge, then the title with the overall score on its own line as stars + figure, the DLC pack when the rating is for one, **all four scored axes in full** (difficulty / grind / fun / hours) as key-figure-verdict cells, the quick take behind the plat card's big opening quote — upright and sized like the game page's blurbs, with **three lines reserved whether or not there is one** so a long take cannot inflate every card in its grid row — and their score against the community's. |
...
- **Confirming a save is the CONTROLLER's job**, not each host's. `RatingFields` toasts on success unless the host sets `announcesSave` (Game Detail and the wizard do, because they word it themselves — the wizard names the game, since the card has already been replaced by the time you read it). The plat-card modal had no toast at all: it saved, closed, and the only sign was a preview redrawing a moment later. The asymmetry behind it was that the controller's *error* path already fell back to a toast while its *success* path delegated entirely — so forgetting an error handler was harmless and forgetting to confirm a save was silent.
- **The prefill shape has ONE definition per side.** `UserConceptRating.as_prefill()` on the server, `PlatPursuit.RatingFields.prefillFrom()` in the client. It used to be hand-built in four places — the wizard queue, the plat-card service, `applyRating` in game-detail.js and `onSaved` in plat-cards.js — and when `recommendation` was added only one was updated. The symptom was near-invisible: game detail prefilled the verdict on first open (the server renders that `data-existing` attribute) and then blank after any save until a reload; the share modal never prefilled it at all. A field missing from a prefill payload does not error, it just arrives empty — and on an edit, an omitted field can be written back as its default.
