...
- **Shovelware and ratings**: Shovelware is NO LONGER gated at the rating API. A shovelware platinum is a real platinum, so `_get_concept_and_group` (rate POST + trophy list) accepts it, and the share-card / dashboard / game-detail rate prompts no longer skip shovelware. The Rate My Games wizard still HIDES shovelware from its queue by default: `ReviewHubService.get_ratable_concept_ids(profile, include_shovelware=False)` excludes it, and `WizardQueueView` re-includes it only when `?include_shovelware=1` (the wizard's "Include shovelware" checkbox). The unrated-count badges stay shovelware-free (they call the helper with the default). The dormant `/reviews/` Review Hub detail view keeps its own all-shovelware 404 gate, independent of the rating API.
