...
- **The share card may only be resized by `transform`, never by layout.** It is a fixed 1200x630 render whose internals are positioned for exactly that size, mounted into a flex frame (`.rcx__card-frame`) that scales it with `--rcx-card-scale`. The frame's child therefore carries `flex: none`: with the default `flex-shrink: 1` a narrow stage squeezed the card's *width* while its inline height held (1200x630 became 343x630 on a phone), so the internals re-laid out at a third of their design width and overlapped each other, and `fitCard` then computed its scale from the wrong natural size. Nothing about the end state shows this -- a card was present and a scale was applied.
- **`badges_data` is a snapshot in whatever shape the generator used at the time, and the beat drops rows it cannot draw.** Recaps are sealed at generation, so rows written before `get_badge_stats_for_month` moved onto `UserGroupBadge` still hold the legacy payload (`name`/`tier_name`/`image_url`) rather than the Medallion frame dict (`art_layers`/`state`/`series_name`/`badge_name`). The slide feeds each entry straight to `components/badge_medallion.html`, which draws from `art_layers`, so a legacy row would render an empty medallion with blank captions. The `badges` beat therefore filters on `art_layers`; the stored count and XP survive the filter, so a pre-rework month still reports "2 badges this month" with its stamp, just without the art.
- **Never "backfill" a historical recap's badges.** It looks like the tidy fix and it is unsound twice over. `UserGroupBadge.earned_at` is the CURRENT-iteration completion date -- it moves when a series gains a game, and the row is deleted outright when the bar rises -- so re-running `get_badge_stats_for_month` over a past window returns whichever badges sit there *today*, not what the hunter earned then. A month with two real badges can regenerate to zero. Separately, `get_or_generate_recap` returns cached data on `is_finalized` *before* it consults `force_regenerate`, and every past month is finalized the moment it is generated, so a backfill would first have to dismantle the immutability the snapshot exists to provide.
- **Hero titles are phrases wearing a number's clothes.** `.pp-tally--hero` is sized for one to three glyphs, but `intro`, `taste` and `summary` use it for text (the month, the top genre, "That's a wrap"). On phones those are capped to the band between the nav arrows (`max-width: calc(100vw - 8.5rem)`, the arrows being 3rem wide at a 1rem inset) so a long title wraps *inside* the band instead of running underneath the arrows. The cap is a width and deliberately not a smaller font: "February" already fitted, so stepping the size down would cost the opening beat its scale to fix a different slide's problem.
