...
| `2324440264` | ppc-roadmap-detail | Trophy roadmap/guide detail, below metrics strip / above content+TOC split | `templates/trophies/roadmap_detail.html` |
...
- **CSP breaks silently if a new CMP or AdSense domain is introduced.** Google periodically adds new ad-serving subdomains. The browser-console error under `Content-Security-Policy` is the user-visible tell, but for passive monitoring use the staff dashboard at `/staff/csp-violations/` — every blocked load is POSTed to `/csp-report/` by the browser and buffered in Redis (`csp:violations`, rolling 1000 entries, 30-day TTL). When violations show up, add the named domain to the directive named in the report. CSP lives in `settings.py` under `CONTENT_SECURITY_POLICY`.
- **The `ad_unit.html` partial renders the wrapper visible from the start** (empty flex, max-height 150px, no min-height, so it collapses to 0 height when no children). Do NOT add `display:none` to the wrapper's initial style: AdSense's responsive sizing reads `availableWidth=0` from a hidden parent and bails with `TagError: No slot size for availableWidth=0`, leaving the slot permanently unfilled with no `data-ad-status` set. On `data-ad-status="filled"` the MutationObserver adds an `my-6` margin; on `data-ad-status="unfilled"` it sets `display:none` to collapse any residual AdSense iframe. If you're debugging "ads not showing," check the browser console for that TagError and inspect the `<ins>` for `data-ad-status`.
