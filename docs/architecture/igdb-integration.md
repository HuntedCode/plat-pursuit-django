...
Franchise and collection data follows the same normalization pattern. A single `Franchise` record represents either an IGDB franchise ("Resident Evil") or an IGDB collection ("Resident Evil Main Series"), distinguished by `source_type`. Links go through a `ConceptFranchise` table with an admin-curated `is_excluded` flag (rare bad-link override) and an IGDB-derived `is_spinoff` flag (collection-only). Every non-excluded link is equal — there's no "primary franchise" anymore. Franchises and collections are **separate IGDB namespaces** — franchise id 222 is "NCAA", collection id 222 is "Army of Two" — so the `Franchise` model uses a composite `(igdb_id, source_type)` unique constraint, never `igdb_id` alone. The franchise browse page at `/franchises/` surfaces franchises that have at least one non-excluded linked concept, plus collections that contain at least one game with no franchise-type link (so collection-only series like Astro Bot remain discoverable). See [Franchise System](../features/franchise-system.md) for the user-facing feature.
...
| **Per-platform release dates** | IGDBMatch | `igdb_ps_release_dates` | Sorted list capturing **every distinct PS-platform release_dates entry** IGDB returns, including multi-region rows and multi-stage rows (alpha, beta, full release). Format: `{platform: <int>, date: "YYYY-MM-DD", status: <int|null>, region: <int|null>}`. `status` is the IGDB release_date_status enum (1=Release, 2=Alpha, 3=Beta, 4=Early Access, 5=Offline, 6=Cancelled, 7=Rumored, 8=Delayed) and is `null` on rows enriched before the field was added to the IGDB query (run `enrich_from_igdb --refresh` to populate). Display surfaces (review tools, future user-facing pages) collapse this to one-row-per-platform via `IGDBService.collapse_ps_release_dates_for_display`, which prefers `status=Release` entries and falls back to a cross-platform-coincidence heuristic when status is missing. The matcher's any-PS-date proximity boost reads the full list directly (catches cross-gen remasters where PSN's date matches a later platform launch). Empty list when IGDB has no per-platform `release_dates` entries. |
...
| **Franchises / collections** | Franchise + ConceptFranchise | `name`, `slug`, `source_type`, `is_excluded`, `is_spinoff` | All IGDB-listed franchises + collections become equal links; `is_excluded` is an admin override for the rare bad link; `is_spinoff` marks games IGDB types as a spin-off of a series (fetched from `/collection_memberships`) |
...
### ConceptFranchise
M2M through table. Links Concept to Franchise. All IGDB-listed franchises a game belongs to are equal links — there's no "primary franchise" anymore. Two admin-curated flags:

- `is_excluded` (default False): admin override that hides a specific link from every consumer (browse list, franchise detail, game detail, franchise-tier badge coverage). Use for the rare case where IGDB lists a game under a franchise it shouldn't (cameo, cross-reference). The IGDB writer NEVER touches this field, so it's safe from enrichment refresh — but the row itself gets wiped+recreated on every refresh unless `concept.franchises_locked=True`, so the lock is what makes an exclusion sticky.
- `is_spinoff` (default False): IGDB-derived for COLLECTION links only. True when IGDB types this game's membership in a collection as a "Spin-off" (membership type 2) rather than a normal "Member" — collection links only, always False for franchise links. The flag is per-(game, collection), so it lives on the link, not the Concept or Franchise (a game can be a Member of one series and a Spin-off of another). Admins can manually flip it via the inline; survives enrichment refresh under the lock.

Unique on (concept, franchise); partial index on `is_excluded=True` (the rare case); composite index on `(franchise, is_spinoff)` for the spin-off suppression queries. Both flags travel intact through `Concept.absorb()` — on duplicate-franchise merges, EITHER side's `True` wins (deliberate curation should never be silently lost).
...
`IGDBService._create_concept_franchises` walks the IGDB response's `franchise` (singular), `franchises` (plural), and `collections` fields and creates one normalized link per entry. All non-excluded links are equal — the writer does NOT designate a primary franchise. Dedup key is `(igdb_id, source_type)` because IGDB franchises and collections live in separate ID namespaces.

The writer:
- Creates `Franchise` rows via `get_or_create(igdb_id, source_type)` with the IGDB name + a slug. Handles slug collisions across namespaces by retrying with a disambiguated `{slug}-{igdb_id}`.
- Creates `ConceptFranchise` rows via `get_or_create(concept, franchise)` with `is_spinoff` derived from the `/collection_memberships` endpoint (live path only).
- NEVER touches `is_excluded` on existing rows — that's admin territory; clobbering on every refresh would defeat the point of the override.
- Skips entirely when `concept.franchises_locked=True` so manual curation is sticky.
...
2. `_job_sync_complete` drains the set at its `igdb_enrich` phase (runs before the health check) and calls `_try_igdb_enrich(concept)` for each queued concept. This step is **per-game anchoring**, not concept-level enrichment: for each Game in the PSN concept, it runs `match_game(game)` (the canonical trophy-group-title signal) and routes the Game to its per-version IGDB-anchored Concept via `anchor_game_to_canonical(game, raw_id, raw_data=...)`. The IGDB game record from `match_game` is threaded through so `anchor_game_to_canonical` doesn't re-fetch the same id. The PSN concept absorbs away via `add_concept`'s cascade once empty.
3. When no Game in the concept matches per-game, `_try_igdb_enrich` falls back to concept-level `IGDBService.enrich_concept(concept)` (uses `match_concept` against `unified_title`) so true compilations whose individual game titles miss but whose collection title hits IGDB still get an IGDBMatch. If that also misses, it records `no_match` so the weekly retry cron (`enrich_from_igdb --missing-or-no-match`) picks the concept up later.

The deferral matters because the early `try_anchor_new_game` call in `_job_sync_title_id` runs before `sync_trophies` populates trophy groups — `_extract_trophy_group_title` returns empty, `match_game` returns None, and every new Game falls through to a PSN concept. The actual anchoring has to wait until trophy data is in place, which is why it's drained at sync_complete and not inline.
...
- **Concept.absorb() ties IGDB enrichment to the IGDBMatch**: IGDB-derived through-rows (ConceptCompany, ConceptGenre, ConceptTheme, ConceptEngine, ConceptFranchise) are a deterministic projection of the concept's IGDBMatch, so they migrate ONLY when the target inherits the source's match (i.e. the target had no match of its own — `inherit_match`). When the target keeps its own match, the source's enrichment belongs to a different IGDB game and is dropped (cascade-deletes with the source), not merged. On the inherit path companies merge roles (OR of flags) and genres/themes/engines/franchises dedup by their respective id. Merging these unconditionally was the re-anchor bug: re-pointing an erroneously-matched concept stacked both matches' developers/genres/franchises onto the survivor. Any NEW model with an FK to Concept must be added to `absorb()` too — and if it is IGDB-derived, gate it on `inherit_match`.
...
- **`is_excluded` survives only under the lock**: `ConceptFranchise.is_excluded=True` is an admin override that hides a specific link from browse / detail / badge coverage. The IGDB writer doesn't touch the column, but the row itself is wiped+recreated on every enrichment refresh of an unlocked concept (`_wipe_concept_enrichment` deletes all ConceptFranchise rows, then `_create_concept_franchises` recreates them with `is_excluded=False`). The exclusion is sticky ONLY when `concept.franchises_locked=True`. Document this when staff sets an exclusion; otherwise it'll vanish on the next refresh.
...
- **`Concept.release_date` vs. `Game.platform_release_date`**: `Concept.release_date` is sourced from `IGDBMatch.igdb_first_release_date`, which is the earliest PS release across ALL versions of the concept. On a per-Game detail page that's misleading: a PS5-only re-release with a PS3 sibling shows the PS3 date. Use `Game.platform_release_date` (property on `Game`) instead — it filters `igdb_ps_release_dates` by `Game.title_platform`, prefers `status=Release` entries, and falls back to `Concept.release_date` when no platform-specific data is available. Requires `concept__igdb_match` to be prefetched on the caller's queryset.
...
- **`involved_companies` can list the same company twice**: IGDB sometimes splits a single studio's roles across multiple rows (e.g. one row with `developer=True`, a second row with `publisher=True`, both pointing at the same company id). `_create_concept_companies` pre-aggregates by IGDB company id and OR-merges the four role flags before writing, so a studio that both develops and publishes ends up with both flags True on its single ConceptCompany row. Walking through the rows naively with `update_or_create` would silently lose whichever role appeared first; that bug shipped briefly in 2026 and required a `rebuild_concept_enrichment` pass to repair.
...
| `rebuild_franchises_from_cache` | Rebuild Franchise + ConceptFranchise rows from cached `IGDBMatch.raw_response`. No IGDB API calls. `--wipe` deletes existing rows first (locked concepts preserved by default). `--force` bypasses the lock when curated data is also corrupted. | `python manage.py rebuild_franchises_from_cache --wipe` |
