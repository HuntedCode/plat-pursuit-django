...
| **Game lists** | Fired on the `is_public` false→true flip, only when `game_list.game_count > 0` (don't surface empty lists). **The cited call site is gone** -- `api/game_list_views.py` was unrouted in 2026-08 and the 2026-09 rebuild fires nothing. The equivalent flip now happens in `gamelists.services.game_list_service.update_list`, which is the one writer and therefore the right place to emit from. | `gamelists/services/game_list_service.py` (unwired) |
