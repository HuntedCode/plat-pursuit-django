...
| `trophies/services/contract_service.py` | Contract XP engine: detection (`mark_contract_reached` / `check_profile_contracts`), acceptance (`accept_contract` / `accept_contracts`), reconciliation (`credit_is_orphaned` / `revoke_contract` -- the only subtractive path), `claimable_contracts`, `recompute_profile_job_xp` |
...
**Contract / job XP engine:**
- **The ledger is immutable, never recomputed from config** (though a revoke may DELETE rows -- see the ratchet bullet): `ContractXPGrant` rows record the amount *as paid* (with `base_t` + `multiplier`). Changing `CONTRACT_XP_TOTAL` or a Contract's jobs later does NOT retroactively change banked XP. `recompute_job_xp` rebuilds only the *cache* (`ProfileJobXP`) by re-summing the existing ledger; it never re-derives amounts.
- **Pursuer Level floors every untouched job at 1**: it is the sum of per-job levels across the WHOLE ~25-job catalogue, not over the `ProfileJobXP` rows that happen to exist (a row is only materialized once a job is paid). `PURSUER_RANKS` is calibrated against that floor ("Pursuer Level ~= 25 floor + 2 per game"), so an unfloored figure reads a rank or two low. One definition, `contract_service.pursuer_level_from`, used by the live read (`_pursuer_level`), the materialized `ProfileCareerStanding.pursuer_level`, the display (`job_render.build_profile_jobs`), and the `pursuer_level` milestone metric (`milestones/metrics.py`, whose Pursuer Ascent rungs are calibrated on the same floor). They drifted once -- the standing summed its rows while the Career page floored -- so the leaderboard's "level" column and the hunter's own hero showed different numbers. **Changing the definition requires `recompute_job_xp --all`**; the standing is materialized and old rows keep the old rule.
...
- **Reached is a ratchet against COMPLETION, not against MEMBERSHIP; accept is once-per-tier**: a 100%'d game that later drops below 100% (new DLC) keeps its reached/accepted state and its banked XP. The one exception is reconciliation: if a Concept split / re-anchor means the hunter's completion no longer belongs to the Contract at all, `reconcile_contracts` revokes the credit and the XP (see [job-board-contracts.md](../design/rebuild/job-board-contracts.md#reconciliation--when-membership-changes-under-a-hunter)). Completion regressions never revoke; membership drift does. Re-detect + re-accept grants nothing more (idempotent via the accepted timestamps + the ledger `unique_together`). Surfaces that show "claimable" status must read `EarnedContract`, not live `ProfileGame.progress`.
...
| `reconcile_contracts` | Revoke credit current membership no longer supports (after a Concept split / re-anchor). Preview by default; `--apply` writes. Refuses a zero-member contract without `--force-empty`. Never on cron. | `python manage.py reconcile_contracts --contract <slug>` |
