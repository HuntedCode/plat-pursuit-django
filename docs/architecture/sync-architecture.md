...
- **Every drift gate asks "is there MORE than before?", which makes a PSN *retraction* invisible to all of them.** `has_drift` compares earned counts, `needs_groups` compares defined totals, and the per-title `ProfileGame` upsert used to compare `last_updated_datetime`. When Sony revises a title's numbers DOWNWARD in place, all three read "nothing changed" and the stale row survives indefinitely. The Long Dark (NPWR13317_00) reported earned=88 against defined=76 during an April 2026 list restructure, which produced a grade-weighted `progress` of 111. Sony later corrected the payload to 76/76 at progress 100 without touching `last_updated_datetime`, so for five months every sync fetched the correction and threw it away. `create_or_update_profile_game` now compares the PSN-owned VALUES rather than the timestamp, and `_sane_progress` clamps an out-of-range percentage into 0-100 while logging both totals (the overshoot is the diagnosis: it means the title's trophy list moved under Sony). The stakes are higher than a wrong-looking number, because `progress == 100` is the completion test in roughly a dozen places (`ProfileGameManager.completed()`, badge `full_complete`, contract completion, the plat-card gate), so an out-of-range row silently denies a hunter credit for a game they finished. Note the other half is still open: no sync path prunes `Trophy` / `TrophyGroup` rows for a group PSN has dropped, so our per-group view of such a title keeps showing content PSN no longer serves.
