...
- **Every drift gate asks "is there MORE than before?", which makes a PSN *retraction* invisible to all of them.** `has_drift` compares earned counts, `needs_groups` compares defined totals, and the per-title `ProfileGame` upsert used to compare `last_updated_datetime`. When Sony revises a title's numbers DOWNWARD in place, all three read "nothing changed" and the stale row survives indefinitely. The Long Dark (NPWR13317_00) reported earned=88 against defined=76 during an April 2026 list restructure, which grade-weighted to `progress=111`. Sony later corrected the payload to 76/76 at progress 100 without touching `last_updated_datetime`, so for five months every sync fetched the correction and threw it away. Three things now hold this down, and each exists for its own reason:
  - `create_or_update_profile_game` compares the PSN-owned VALUES rather than the timestamp, and returns the list of fields it wrote.
  - When the payload fails its own arithmetic (`earned_total > defined_total`, or a percentage outside 0-100), our own `EarnedTrophy` denorms outrank it: the stored value becomes the FLOORED `earned / (earned + unearned)`, which reads 100 only when nothing is unearned. Clamping straight to 100 would have been wrong, and was the first version of this fix: an earned COUNT that outnumbers the defined count does not establish set containment, since trophies from a group PSN dropped can outnumber base-game trophies the hunter is still missing, so clamping would invent a completion.
  - The walk adds a row whose `progress` changed to `touched_profilegame_ids` even when no trophy drift sent it for a re-sync, because badge eval and contract detection are scoped to that list. Without it the number gets corrected and the credit it gates does not.
  The stakes are why all three matter: `progress == 100` is an EQUALITY test for badge `full_complete`, contract completion, the plat-card gate, `Profile.total_completes` and `Game.full_completion_count`, so an out-of-range row silently denies a hunter credit for a game they finished. **Two halves remain open.** No sync path prunes `Trophy` / `TrophyGroup` rows for a group PSN has dropped, so our per-group view of such a title keeps showing content PSN no longer serves. And `ProfileGame.earned_trophies` still stores PSN's per-tier counts verbatim, so a corrected row can read 100% while its own blob sums above the game's defined total; that blob feeds the badge-detail trophy haul and the game leaderboard's secondary metric.
