...
- PSNConceptData (bulk re-point, no dedup: rows are keyed on (psn_concept_id, country) and `concept` is not part of that key, so re-pointing cannot collide. Deliberately unlike the IGDB enrichment branch, which drops the source's rows when the survivor has its own match: IGDB enrichment is a projection of ONE match, whereas PSN rows are independent storefront records and one concept legitimately has several. `PSNRawPayload` follows its row via the OneToOne and needs no branch. This is also the ONLY branch wrapped in its own try/except, because it was added after the branches above it and absorb() is not transactional: a raise here would leave everything above already committed, skip everything below, and stop the caller's `other.delete()`. A capture table nothing reads must not be able to half-migrate a merge)
- `gamelists.GameListItem.concept` (added 2026-09 when the rebuilt Game Lists moved from `Game` keying to `Concept` keying, per the Games/Trophy Lists IA). Re-pointed, never left to cascade: a cascade deletes a hunter's hand-curated backlog entry because of a catalogue merge they never saw. **The dedup is load-bearing and cannot be a bare `.update()`**: a list may already hold the survivor, and `unique(game_list, concept)` then raises mid-merge — which, since `absorb()` is not transactional, commits everything above it, skips everything below, and stops the caller's `other.delete()`. Colliding entries are DROPPED (that list already has the game); the rest move. Dedup is scoped per-list, so somebody else's list holding only the doomed concept is still re-pointed.
- `prompts.PromptGame.concept` (added 2026-09 with Tiers, Grids & Polls). Same dedup shape as `GameListItem` and **one step harder**: a pool row is what every RESPONSE's placements point at, so dropping a colliding row cascades into other people's answers. The placements must be re-pointed to the surviving pool row FIRST, excluding responses that already placed the survivor (`unique(response, prompt_game)` would raise, and those are genuine duplicates that should cascade). Three invariants get repaired afterwards, all set-based: pool `position`, `Prompt.game_count`, and `PromptResponse.placement_count` for every response of a colliding prompt. **`PromptPlacement` now needs its OWN branch too** — it FKs `PromptGame` for a pooled prompt (the whole point of that FK) *and*, since open grids shipped, `concept` directly for a free pick on a grid with no pool. The two dedup differently: the pool branch against `unique(response, prompt_game)`, the free-pick branch against a `unique(response, concept)` that is PARTIAL on `no_duplicates` — so a grid allowing duplicates collides with nothing and every row simply re-points.
...
Note: `RoadmapEditLock`, `RoadmapRevision`, `RoadmapNote`, `RoadmapNoteRead`, and the `created_by`/`last_edited_by` FKs on `RoadmapStep`/`TrophyGuide` all FK to `Roadmap` or `Profile`, not `Concept` directly, so they don't need their own absorb branches. They follow `Roadmap` automatically. Likewise `BlurbReport` FKs `UserConceptRating` (the rating's `blurb` field IS the migrated row, and ratings are already handled above), so it follows the rating through absorb (cascades when a rating is deduped, stays attached when re-pointed) and needs no branch of its own.
