/*
 * Trophy Lists BROWSE -- page motion + chrome (the tag-detail.js clone, tlb- ids). Filtering is
 * HTMX-native via browse-filters.js; this owns the header/minibar count ticks, the collapsible
 * Filters panel, the staggered card reveal, infinite scroll, the filter-swap settle, and the
 * sticky mini-bar proxies. One un-condensed card per trophy list (Games/Trophy Lists IA phase 4).
 * Wired via PlatPursuit.onPageReady(boot): element wiring re-runs on load AND on HTMX
 * Back/Forward history restore; body-level listeners are guarded by `first` (playbook section 7).
 */
(function () {
    ...
    function initToolbar() {
        ...
        form = document.getElementById('tlb-form');
        panelHandle = PP.filterPanel && PP.filterPanel({
            form: form,
            toggle: '#tlb-filters-toggle',
            panel: '#tlb-advanced',
            countEl: '#tlb-filter-count',
    ...
    // Filtered-count tick -> BOTH the header Tally (this page's headline IS the filtered
    // count -- no heartbeat scards here) and the mini-bar count.
    function tickCount(grid) {
        ...
        if (headCount) {
            if (PP.countUp) { headCount.dataset.countup = String(newVal); PP.countUp(headCount, 500); }
            else { headCount.textContent = newVal.toLocaleString(); }
        }
    }
    ...
    function onMbFiltersClick() {
        var tg = document.getElementById('tlb-filters-toggle');
