/*
 * Franchise / Series detail -- page motion + the sort swap. The only interactive control is the sort select,
 * which HTMX-swaps just #franchise-groups (the header, your-progress block, and related rail stay put). This
 * owns the header count-ups (franchise totals + your-progress %), the your-progress bar fill-from-0, the
 * staggered group-card reveal, and the .is-swapping settle on a sort change.
 *
...
(function () {
    ...
    function initReveal() {
        ...
        var grid = document.querySelector('#franchise-groups .fgroup-grid');
    ...
    // Settle the group list the instant a sort change fires (HTMX swaps its innerHTML).
    function onSortChange() {
        var g = document.getElementById('franchise-groups');
        if (g) { g.classList.add('is-swapping'); }
    }

    function onAfterSwap(e) {
        var t = (e.detail && e.detail.target) || e.target;
        if (!t || t.id !== 'franchise-groups') { return; }
    ...
    function boot(first) {
        if (first) {
            ...
            // Fill the your-progress bar from 0 on first paint (double-rAF so the 0% width lands before the
            // transition to the real value). Skipped when the user prefers reduced motion.
        ...
        if (form) { form.addEventListener('change', onSortChange); }   // same fn ref -> dedupes across boots
        initReveal();
