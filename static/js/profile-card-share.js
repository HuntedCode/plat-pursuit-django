 ...
 */
// ------------------------------------------------------------------
// Badge Showcase: mode toggle + share card / profile showcase logic
// ------------------------------------------------------------------

// Current mode: 'share_card' or 'showcase'
window.PlatPursuit._badgeMode = 'share_card';
...
window.PlatPursuit.setBadgeDisplayed = async function(badgeId) {
    try {
        ...
        // Update UI: toggle share card selection indicators
        document.querySelectorAll('.badge-showcase-item').forEach(function(item) {
            ...
            if (!isSelected && item.dataset.showcased !== 'true') {
                item.classList.toggle('border-base-content/10', true);
            }

            // Update share card checkmark (only touch .share-card-indicator)
            const check = item.querySelector('.share-card-indicator');
            if (check) check.remove();
            if (isSelected) {
                item.insertAdjacentHTML('beforeend',
                    '<div class="share-card-indicator absolute -top-1 -right-1 w-4 h-4 bg-secondary rounded-full flex items-center justify-center">' +
...
window.PlatPursuit.toggleShowcaseBadge = async function(badgeId) {
    try {
        const result = await PlatPursuit.API.post('/api/v1/badges/showcase/', {
            badge_id: badgeId,
        });

        const showcaseOrder = result.showcase_badge_ids || [];
        const showcaseIds = new Set(showcaseOrder);

        // Update grid indicators
        document.querySelectorAll('.badge-showcase-item').forEach(function(item) {
            const id = parseInt(item.dataset.badgeId);
            const isShowcased = showcaseIds.has(id);
            item.dataset.showcased = isShowcased ? 'true' : 'false';

            // Toggle showcase pin indicator (bottom-right)
            let pin = item.querySelector('.showcase-indicator');
            if (isShowcased && !pin) {
                item.insertAdjacentHTML('beforeend',
                    '<div class="showcase-indicator absolute -bottom-1 -right-1 w-4 h-4 bg-primary rounded-full flex items-center justify-center">' +
                    '<svg xmlns="http://www.w3.org/2000/svg" class="w-2.5 h-2.5 text-primary-content" viewBox="0 0 24 24" fill="currentColor"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></polygon></svg>' +
                    '</div>'
                );
            } else if (!isShowcased && pin) {
                pin.remove();
            }
        });

        // Update showcase slots (pass ordered array, not Set)
        PlatPursuit._updateShowcaseSlots(showcaseOrder);

        const action = result.action === 'added' ? 'added to' : 'removed from';
        PlatPursuit.ToastManager.show(`Badge ${action} profile showcase!`, 'success');
    } catch (err) {
        const errData = await err.response?.json().catch(function() { return null; });
        PlatPursuit.ToastManager.show(
            errData?.error || 'Failed to update showcase.',
            'error'
        );
    }
};

window.PlatPursuit._updateShowcaseSlots = function(showcaseIdOrder) {
    // showcaseIdOrder: array of badge IDs in display order, or Set (legacy)
    const orderedIds = Array.isArray(showcaseIdOrder) ? showcaseIdOrder : [...showcaseIdOrder];
    const slots = document.querySelectorAll('.showcase-slot');

    // Build a lookup from the grid DOM
    const badgeDataById = {};
    document.querySelectorAll('.badge-showcase-item').forEach(function(item) {
        badgeDataById[parseInt(item.dataset.badgeId)] = {
            id: item.dataset.badgeId,
            name: item.dataset.badgeName || '',
            image: item.dataset.badgeImage || '',
        };
    });

    // Gather in API-returned order
    const showcasedItems = orderedIds
        .map(function(id) { return badgeDataById[id]; })
        .filter(Boolean);

    slots.forEach(function(slot, i) {
        const badge = showcasedItems[i];
        if (badge) {
            slot.innerHTML = '<img src="' + PlatPursuit.HTMLUtils.escape(badge.image) + '" class="w-10 h-10 md:w-12 md:h-12 object-contain" />'
                + '<span class="text-[0.6rem] text-base-content/50 text-center line-clamp-1 px-1">' + PlatPursuit.HTMLUtils.escape(badge.name) + '</span>';
            slot.classList.remove('border-dashed', 'border-base-content/10');
            slot.classList.add('border-solid', 'border-primary/30', 'bg-primary/5');
        } else {
            slot.innerHTML = '<span class="text-base-content/20 text-xs font-bold">Slot ' + (i + 1) + '</span>';
            slot.classList.add('border-dashed', 'border-base-content/10');
            slot.classList.remove('border-solid', 'border-primary/30', 'bg-primary/5');
        }
    });
};

// Note: Badge showcase mode toggle and grid click routing are initialized
// by dashboard.js via _initBadgeShowcase() (lazy-loaded module pattern).
