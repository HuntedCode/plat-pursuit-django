...
const GameListEditor = {
    ...
    _activeFilters: { platform: new Set(), region: new Set() },
    ...
    _bindSettingsToggle() {
        ...
        const btnLabel = document.getElementById('settings-toggle-label');
        if (!btn || !panel) return;

        const updateBtnState = (open) => {
            if (open) {
                btn.classList.remove('btn-outline');
                btn.classList.add('btn-active');
                if (btnLabel) btnLabel.textContent = 'Close Settings';
            } else {
                btn.classList.add('btn-outline');
                btn.classList.remove('btn-active');
                if (btnLabel) btnLabel.textContent = 'Settings';
            }
        };
    ...
    _bindGameSearch() {
        ...
        const debouncedSearch = PlatPursuit.debounce(async (query) => {
            ...
            try {
                let url = `/api/v1/games/search/?q=${encodeURIComponent(query)}&limit=10&exclude_list=${this.listId}`;
                if (this._activeFilters.platform.size > 0) {
                    url += `&platform=${[...this._activeFilters.platform].join(',')}`;
                }
                if (this._activeFilters.region.size > 0) {
                    url += `&region=${[...this._activeFilters.region].join(',')}`;
                }
    ...
    _bindSearchFilters() {
        ...
        const updateChipVisual = (chip, active) => {
            ...
            if (active) {
                const colorClass = filterType === 'platform'
                    ? ACTIVE_CLASSES.platform
                    : (ACTIVE_CLASSES.region[value] || 'badge-warning');
        ...
        const updateFilterCount = () => {
            const total = this._activeFilters.platform.size + this._activeFilters.region.size;
            if (filterCountBadge) {
                if (total > 0) {
        ...
        chips.forEach(chip => {
            chip.addEventListener('click', () => {
                ...
                if (this._activeFilters[filterType].has(value)) {
                    this._activeFilters[filterType].delete(value);
                    updateChipVisual(chip, false);
                } else {
                    this._activeFilters[filterType].add(value);
                    updateChipVisual(chip, true);
                }

                updateFilterCount();
                this._retriggerSearch();
            });
        });

        if (clearBtn) {
            clearBtn.addEventListener('click', () => {
                this._activeFilters.platform.clear();
                this._activeFilters.region.clear();
    ...
    _renderSearchResults(games, container) {
        if (!games.length) {
            const hasFilters = this._activeFilters.platform.size > 0 || this._activeFilters.region.size > 0;
            container.innerHTML = `
                <div class="p-4 text-center text-base-content/50">
                    <p class="font-medium">No games found</p>
                    ${hasFilters ? '<p class="text-xs mt-1">Try adjusting your search or filters</p>' : ''}
                </div>`;
        ...
        const html = games.map(game => {
            ...
            // Platinum indicator
            const hasPlatinum = game.defined_trophies?.platinum;
            const platBadge = hasPlatinum
                ? `<span class="badge badge-xs font-bold" style="background-color: var(--color-trophy-platinum); color: #000;">Plat</span>`
                : '';
                ...
                if (container && container.children.length === 0) {
                    container.insertAdjacentHTML('afterend', `
                        <div class="text-center py-12" id="empty-state">
                            ...
                            <p class="text-base-content/50 mt-4 text-lg font-semibold">Your list is a blank canvas</p>
                            <p class="text-base-content/40 text-sm mt-1">Search above and start curating your collection</p>
                        </div>
                    `);
                }
            }
