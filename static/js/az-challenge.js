...
const AZChallengeDetail = {
    ...
    _bindPickNextGame() {
        ...
        const spinner = new PlatPursuit.ReelSpinner({
            ...
            tileLabel:    (s) => s.letter,
            tileIcon:     (s) => s.game_icon,
            tileName:     (s) => s.game_name,
            resultBadge:  (s) => s.letter,
            coverApiUrl:  (id) => `/api/v1/challenges/az/${id}/update/`,
            coverPayload: (s) => ({ cover_letter: s.letter }),
            gridSelector: (s) => `[data-letter="${s.letter}"][data-slot-id]`,
            winnerKey:    (s) => s.letter,
        });
