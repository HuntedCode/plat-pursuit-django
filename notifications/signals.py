...
def send_consolidated_milestone_email(user_milestones):
    """
    Send a single consolidated email for all milestones earned in one sync cycle.

    Accepts a list of UserMilestone instances (the highest tier per criteria type,
    matching the in-app notification consolidation). Builds context for each and
    renders one email listing all achievements.

    Called from token_keeper.py after check_all_milestones_for_user() completes.
    """
    ...
    # All user_milestones belong to the same profile (single sync)
    profile = user_milestones[0].profile
    user = profile.user

    if not user or not user.email:
        return

    milestone_count = len(user_milestones)
    subject = (
        f'Milestone Achieved: {user_milestones[0].milestone.name}'
        if milestone_count == 1
        else f'You achieved {milestone_count} milestones!'
    )

    if not EmailPreferenceService.should_send_email(user, 'milestone_notifications'):
        EmailService.log_suppressed(
            email_type='milestone_achieved',
            user=user,
            subject=subject,
            triggered_by='system',
        )
    ...
    try:
        ...
        for um in user_milestones:
            ctx = _build_milestone_context(um)
        ...
        EmailService.send_html_email(
            ...
            fail_silently=True,
            log_email_type='milestone_achieved',
            log_user=user,
            log_triggered_by='system',
            log_metadata={
                'milestone_count': milestone_count,
                'milestone_names': [um.milestone.name for um in user_milestones],
            },
        )
        logger.info(f"Sent milestone email to {user.email} ({milestone_count} milestones)")
    except Exception as e:
        logger.exception(f"Failed to send milestone email for {profile.psn_username}: {e}")
