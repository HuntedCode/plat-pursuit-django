...
def test_the_scrolled_board_indexes_are_partial_on_the_population():
    ...
    from trophies.models import (
        Profile, ProfileEditionStanding, ProfileRarityStanding, ProfileTrophyStanding, SeriesEditionStanding,
        UserGroupBadge,
    )

    for model, names in (
        (Profile, ('profile_board_idx', 'profile_board_cc_idx')),
        (ProfileRarityStanding, ('prs_board_idx', 'prs_country_board_idx')),
    ...
    # Rarity Score's membership rule is a FULL 1,000 scorable trophies, not "more than none": below that the
    # sum is short by construction. The literal must track `rarity_score.TOP_N`, asserted just below.
    ...
    expected = {
        ...
        (ProfileRarityStanding, 'prs_board_idx'): linked_pp,
        (ProfileRarityStanding, 'prs_country_board_idx'): linked_pp,
    ...
    assert rarity_score.TOP_N == 1000, (
        'rarity_score.TOP_N moved without the pps_* index conditions following it'
    )
