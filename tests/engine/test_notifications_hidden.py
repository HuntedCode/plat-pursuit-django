...
def test_the_url_names_still_reverse():
    """Three staff pages that have nothing to do with notifications reverse `admin_notification_center`
    (the subscription dashboard, the fundraiser admin, the badge reveal). Dropping the name is a
    NoReverseMatch, which is a hard 500 on all three -- so the names outlive the views."""
    ...
    for rel in ('templates/users/admin/subscription_dashboard.html',
        ...
        assert 'admin_notification_center' in src, (
            f'{rel} stopped linking to the notification centre -- if that is deliberate, this guard and '
            f'the kept URL name can both go'
        )
...
def test_the_staff_user_picker_survived_the_withdrawal(client):
    ...
    assert client.get('/api/v1/admin/notifications/user-search/?q=x').status_code != 404
...
def test_no_url_conf_imports_a_view_it_no_longer_routes():
    ...
    for rel in ('plat_pursuit/urls.py', 'api/urls.py'):
        ...
        dead = sorted(i for i in imported - used if 'Notification' in i)
        assert not dead, f'{rel} imports {dead} but routes none of them'


def test_the_producers_are_untouched_and_still_writing():
    """The point of hiding rather than removing, and the half a rebuilt system keeps. Every producer still
    reaches the model through the one chokepoint, so a row written here proves the whole chain is live."""
...
def test_the_signals_are_still_registered():
    """`NotificationsConfig.ready()` is what wires the producers to their triggers. Pulling the app from
    INSTALLED_APPS would silence every one of them without touching a producer file."""
    from django.conf import settings

    assert 'notifications.apps.NotificationsConfig' in settings.INSTALLED_APPS
